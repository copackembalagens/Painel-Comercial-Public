...
function abaComissao(container) {
  ...
  toggle.innerHTML = `
    ...
    <button type="button" data-view="detalhe" class="${ESTADO.extratoView === "detalhe" ? "ativa" : ""}">Detalhe por parcela recebida</button>
  `;
  ...
  if (ESTADO.extratoView === "comissao") {
    ...
    renderTabela(container, {
      id: "comissao-detalhe", titulo: `Detalhe por parcela recebida (${rotuloPeriodo(detalheComissaoFiltrado)})`,
      colunas: [
        ...(window.AREA_CONFIG.vendedorFiltro ? [] : [{ chave: "vendedor", rotulo: "Vendedor" }]),
        { chave: "mes", rotulo: "Mês" },
        { chave: "nota_fiscal", rotulo: "Nota Fiscal (se houver)" },
    ...
    if (!detalheComissaoFiltrado.length) {
      ...
      aviso.textContent = "Sem detalhe por parcela disponível para este período (meses cobertos pela planilha histórica de comissão ainda não têm detalhamento nota a nota).";
